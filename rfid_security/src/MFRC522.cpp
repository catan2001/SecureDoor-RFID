...
void MFRC522::PCD_ReadRegister(	uint8_t reg,		///< The register to read from. One of the PCD_Register enums.
  ...
  values[index] = bcm2835_spi_transfer(0);			// Read the final uint8_t. Send 0 to stop reading.
} // End PCD_ReadRegister()
 ...
 * 
 */
const string MFRC522::GetStatusCodeName(uint8_t code	///< One of the StatusCode enums.
						      ) {
  switch (code) {
 ...
 * @return PICC_Type
 */
uint8_t MFRC522::PICC_GetType(uint8_t sak		///< The SAK uint8_t returned from PICC_Select().
			   ) {
 ...
 * 
 */
const string MFRC522::PICC_GetTypeName(uint8_t piccType	///< One of the PICC_Type enums.
						     ) {
  switch (piccType) {
